...
class MyObject {
    ...
    setRotation(PHI, THETA, r) {
        this.rotate[0] = PHI;
        this.rotate[1] = THETA;
        this.rotate[2] = r;
        this.child.forEach(obj => {
            obj.setRotation(PHI, THETA, r);
        });
    }
    ...
    generateSin(time, period, rate){
        return  Math.sin((( time % period) / period) * Math.PI * 2) * rate;
    }

    movingLeg(maxMove, time, period, rate){
        let move = this.generateSin(time, period, rate);
        if (move > maxMove){
            move = maxMove;
        }
        return move;
